 ...
 * This information is injected as a single SQS message attribute called "span_context", and encoded as a simple JSON structure.
 *
...
public final class SQSTracing {
     ...
     *
     * @param request The original request to injectInto the span context into. This request will get modified by this method.
     *
     * @return The original request, with the span context injected into it.
     */
    public com.amazonaws.services.sqs.model.SendMessageRequest injectInto(final com.amazonaws.services.sqs.model.SendMessageRequest request) {
     ...
     * @return The original request, with the span context injected into it.
     */
    public com.amazonaws.services.sqs.model.SendMessageRequest injectInto(final com.amazonaws.services.sqs.model.SendMessageRequest request, final SpanContext spanContext) {
        if (spanContext != null) {
            Map<String, String> contextMap = new HashMap<>();
            tracer.inject(spanContext, TEXT_MAP, new TextMapInjectAdapter(contextMap));
            request.addMessageAttributesEntry(
                attributeKey,
                new com.amazonaws.services.sqs.model.MessageAttributeValue().withDataType(STRING_DATA_TYPE).withStringValue(jsonEncode(contextMap)));
        }
     ...
     *
     * This lets you injectInto different span contexts into each individual message entry for a whole batch of messages.
     * If you want to injectInto the same span context into all the message entries of your batch, you can use the
     * {@link #injectInto(com.amazonaws.services.sqs.model.SendMessageBatchRequest)} method instead.
     *
     * @param request The original request entry to injectInto the span context into. This entry will get modified by this method.
     * @param spanContext The span context to injectInto. If null, this method will do nothing.
     *
     * @return The original request entry, with the span context injected into it.
     */
    public com.amazonaws.services.sqs.model.SendMessageBatchRequestEntry injectInto(final com.amazonaws.services.sqs.model.SendMessageBatchRequestEntry request, final SpanContext spanContext) {
        if (spanContext != null) {
            Map<String, String> contextMap = new HashMap<>();
            tracer.inject(spanContext, TEXT_MAP, new TextMapInjectAdapter(contextMap));
            request.addMessageAttributesEntry(
     ...
     * @return The original request entry, with the span context injected into it.
     */
    public com.amazonaws.services.sqs.model.SendMessageBatchRequest injectInto(final com.amazonaws.services.sqs.model.SendMessageBatchRequest request) {
        return injectInto(request, activeContext().orElse(null));
    }
     ...
     * @return The original request entry, with the span context injected into it.
     */
    public com.amazonaws.services.sqs.model.SendMessageBatchRequest injectInto(final com.amazonaws.services.sqs.model.SendMessageBatchRequest request, final SpanContext spanContext) {
        if (spanContext != null) {
            Map<String, String> contextMap = new HashMap<>();
            tracer.inject(spanContext, TEXT_MAP, new TextMapInjectAdapter(contextMap));
     ...
     * but instead returns a new request that is derived from the original one, with the added span context attribute injected.
     *
    ...
    public SendMessageRequest injectInto(final SendMessageRequest request, final SpanContext spanContext) {
        ...
        return request.toBuilder().messageAttributes(tracedAttributes(request.messageAttributes(), spanContext)).build();
    }
     ...
     * but instead returns a new entry that is derived from the original one, with the added span context attribute injected.
     *
     * This lets you injectInto different span contexts into each individual message entry for a whole batch of messages.
     * If you want to injectInto the same span context into all the message entries of your batch, you can use the
     * {@link #injectInto(SendMessageBatchRequest)} method instead.
     *
     ...
     */
    public SendMessageBatchRequestEntry injectInto(final SendMessageBatchRequestEntry request, final SpanContext spanContext) {
        if (spanContext == null) {
            return request;
        }
    ...
    public SendMessageBatchRequest injectInto(final SendMessageBatchRequest request, final SpanContext spanContext) {
        ...
        List<SendMessageBatchRequestEntry> tracedEntries = request.entries().stream()
     ...
     *
     * @return A map of message ids span contexts. For each message id, if a span context was found for the respective message,
     *         then that span context is contained in this map, and referenced by the message id.
     ...
     *
     * Unfortunately, the OpenTracing API decided to throw a runtime exception in case of a failure to parse, which is very error-prone.
     ...
     *
     * @return A map without any optional values inside, with all keys deleted that had empty values in the original map.
     */
    private static <T> Map<String, T> flattenMap(final Map<String, Optional<T>> mapWithOptionals) {
        Map<String, T> flattened = new HashMap<>();
     ...
     */
    private Map<String, MessageAttributeValue> tracedAttributes(final Map<String, MessageAttributeValue> attributes, final SpanContext spanContext) {
        Map<String, MessageAttributeValue> tracedAttributes = new HashMap<>(attributes);
        Map<String, String> contextMap = new HashMap<>();
        tracer.inject(spanContext, TEXT_MAP, new TextMapInjectAdapter(contextMap));
        tracedAttributes.put(attributeKey, MessageAttributeValue.builder().dataType(STRING_DATA_TYPE).stringValue(jsonEncode(contextMap)).build());
     ...
     *
     * As the json encoding is completely under control of this library, exceptions during json parsing will simply return an empty value.
     *
     ...
     *
     * As the json encoding ins completely under control of this library, exceptions during json processing will simply lead to
