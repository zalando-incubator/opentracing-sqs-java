...
public class SQSTracingTestForAws2 {
    ...
    public void producerMustLeaveRequestUnchangedWhenNoContextIsActive() {
        ...
        SendMessageRequest traced = new SQSTracing(mockTracer).injectInto(request);
    ...
    public void producerMustLeaveRequestUnchangedForNullContext() {
        ...
        SendMessageRequest request = request();
        SendMessageRequest traced = new SQSTracing(mockTracer).injectInto(request, null);
    ...
    public void producerMustAddSpanContextToSingleMessage() throws IOException {
        ...
        SendMessageRequest request = request();
        SendMessageRequest traced = new SQSTracing(mockTracer, customName).injectInto(request);

        assertEquals("message must have one additional attribute", 2, traced.messageAttributes().size());
        ...
        MessageAttributeValue attribute = traced.messageAttributes().get(customName);
        assertEquals("attribute data type must be string", "String", attribute.dataType());

        String context = attribute.stringValue();
    ...
    public void producerMustAddExplicitSpanContextToSingleMessage() throws IOException {
        ...
        SendMessageRequest request = request();
        SendMessageRequest traced = new SQSTracing(mockTracer).injectInto(request, inactive.context());
        ...
        MessageAttributeValue attribute = traced.messageAttributes().get(defaultContext);
        assertEquals("attribute data type must be string", "String", attribute.dataType());
    ...
    public void producerMustLeaveBatchEntryUnchangedForNullContext() {
        ...
        SendMessageBatchRequestEntry traced = new SQSTracing(mockTracer).injectInto(request, null);
    ...
    public void producerMustAddSpanContextToBatchEntry() throws IOException {
        ...
        SendMessageBatchRequestEntry request = entry();
        SendMessageBatchRequestEntry traced = new SQSTracing(mockTracer).injectInto(request, mockTracer.activeSpan().context());
    ...
    public void producerMustLeaveBatchRequestUnchangedWhenNoContextIsActive() {
        ...
        SendMessageBatchRequest traced = new SQSTracing(mockTracer).injectInto(request);
    ...
    public void producerMustLeaveBatchRequestUnchangedForNullContext() {
        ...
        SendMessageBatchRequest request = batch();
        SendMessageBatchRequest traced = new SQSTracing(mockTracer).injectInto(request, null);
    ...
    public void producerMustAddSpanContextToWholeMessageBatch() {
        ...
        SendMessageBatchRequest request = batch();
        SendMessageBatchRequest traced = new SQSTracing(mockTracer).injectInto(request, inactive.context());

        traced.entries().forEach(entry -> {
            assertEquals("message must have one additional attribute", 2, entry.messageAttributes().size());
            asList(defaultContext, "preexisting").forEach(key ->
                assertTrue("message attributes contain " + key, entry.messageAttributes().containsKey(key))
            );

            MessageAttributeValue attribute = entry.messageAttributes().get(defaultContext);
            assertEquals("attribute data type must be string", "String", attribute.dataType());
    ...
    public void consumerCanRetrieveSpanContextFromMessages() {
        ReceiveMessageResponse response = ReceiveMessageResponse.builder().messages(
            Message.builder().messageId("id-1").messageAttributes(simpleAttributes("span_ctx", "{\"spanid\":\"3\",\"traceid\":\"5\"}")).build(),
            Message.builder().messageId(("id-2")).build() // no span context in this one
        ).build();

        Map<String, SpanContext> spanContexts = new SQSTracing(mockTracer).extractFrom(response);
